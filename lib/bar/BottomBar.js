...
export default class BottomBar extends React.Component {
  ...
  _renderDownloadButton() {
    const { displayDownloadButton, onDownloadAction } = this.props;
    if (displayDownloadButton) {
      return (
        <TouchableOpacity style={[styles.button, { flex: 0 }]} onPress={onDownloadAction}>
          <Icon name="ios-download-outline" size={25} color="#fff" />
        </TouchableOpacity>
      );
    }
    return null;
